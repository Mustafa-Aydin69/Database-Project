...
class _FlightsScreenState extends State<FlightsScreen> {
  ...
  // --- SABİT LİSTELER ---
  final List<String> _airlines = ['Turkish Airlines', 'Pegasus Airlines', 'AnadoluJet', 'SunExpress'];
  final List<String> _aircrafts = ['Boeing 737-800', 'Airbus A320', 'Boeing 777-300ER', 'Airbus A350'];
  final List<String> _airports = [
    'IST - İstanbul Havalimanı', 'SAW - Sabiha Gökçen', 'AYT - Antalya Havalimanı',
    'ADB - İzmir Adnan Menderes', 'ESB - Esenboğa', 'TZX - Trabzon', 'JFK - New York JFK'
  ];
  final List<String> _statuses = ['Scheduled', 'Delayed', 'Cancelled', 'Boarding', 'Departed', 'Arrived'];
  ...
  List<Map<String, dynamic>> get _filteredFlights {
    return _flights.where((f) {
      final matchesSearch = f['departureAirport'].toString().toLowerCase().contains(_searchTerm.toLowerCase()) ||
          f['arrivalAirport'].toString().toLowerCase().contains(_searchTerm.toLowerCase()) ||
          f['airline'].toString().toLowerCase().contains(_searchTerm.toLowerCase());

      final matchesAirline = _filterAirline.isEmpty || f['airline'] == _filterAirline;
      final matchesStatus = _filterStatus.isEmpty || f['status'] == _filterStatus;
      ...
      builder: (context) => AlertDialog(
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: "Havayolu", border: OutlineInputBorder()),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: "Uçak", border: OutlineInputBorder()),
                          items: _aircrafts.map((a) => DropdownMenuItem(value: a, child: Text(a))).toList(),
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: "Kalkış Havalimanı", border: OutlineInputBorder()),
                          items: _airports.map((a) => DropdownMenuItem(value: a, child: Text(a, overflow: TextOverflow.ellipsis))).toList(),
                          onChanged: (val) => setState(() => _selectedDepAirport = val),
                          validator: (v) => v == null ? "Seçiniz" : null,
                          isExpanded: true,
                        ),
                      ),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: "Varış Havalimanı", border: OutlineInputBorder()),
                          items: _airports.map((a) => DropdownMenuItem(value: a, child: Text(a, overflow: TextOverflow.ellipsis))).toList(),
                          onChanged: (val) => setState(() => _selectedArrAirport = val),
                          validator: (v) => v == null ? "Seçiniz" : null,
                          isExpanded: true,
                        ),
                      ),
                    ...
                    children: [
                      Expanded(
                        child: InkWell(
                          ...
                          child: InputDecorator(
                            decoration: const InputDecoration(labelText: "Kalkış Zamanı", border: OutlineInputBorder(), suffixIcon: Icon(Icons.calendar_today)),
                            child: Text(_departureDate == null ? "Seçiniz" : DateFormat('yyyy-MM-dd HH:mm').format(_departureDate!)),
                          ),
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: InkWell(
                          ...
                          child: InputDecorator(
                            decoration: const InputDecoration(labelText: "Varış Zamanı", border: OutlineInputBorder(), suffixIcon: Icon(Icons.calendar_today)),
                            child: Text(_arrivalDate == null ? "Seçiniz" : DateFormat('yyyy-MM-dd HH:mm').format(_arrivalDate!)),
                          ),
                        ),
                      ),
                    ],
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              if (_formKey.currentState!.validate() && _departureDate != null && _arrivalDate != null) {
                setState(() {
                  if (flight != null) {
                    final index = _flights.indexWhere((f) => f['flightID'] == flight['flightID']);
                    _flights[index] = {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                const Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Uçuş Yönetimi", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.black87)),
                    SizedBox(height: 4),
                    Text("Tüm uçuşları görüntüleyin ve yönetin", style: TextStyle(color: Colors.grey)),
                  ],
                ),
                ElevatedButton.icon(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 12), hintText: "Tüm Havayolları"),
                          items: [
                            const DropdownMenuItem(value: "", child: Text("Tüm Havayolları")),
                            ..._airlines.map((a) => DropdownMenuItem(value: a, child: Text(a))),
                          ],
                          onChanged: (val) => setState(() => _filterAirline = val ?? ""),
                        ),
                      ),
                      SizedBox(width: isMobile ? 0 : 16, height: isMobile ? 16 : 0),
                      SizedBox(
            ...
            if (currentData.isEmpty)
              const Center(child: Padding(padding: EdgeInsets.all(40), child: Text("Uçuş bulunamadı.")))
            else
              LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return _FlightCard(
                        ...
                        onEdit: () => _showFlightDialog(flight: currentData[index]),
                        onDelete: () => _deleteFlight(currentData[index]['flightID']),
                      );
                    },
                  );
                },
...
class _FlightCard extends StatelessWidget {
  ...
  const _FlightCard({required this.flight, required this.onEdit, required this.onDelete});
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'Boarding': return Colors.purple;
      case 'Departed': return Colors.teal;
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(flight['airline'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15), overflow: TextOverflow.ellipsis),
                    Text(flight['aircraft'], style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                  ],
                ),
              ),
              Container(
                ...
                child: Text(flight['status'], style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: statusColor)),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(flight['departureAirport'], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis),
                    Text(DateFormat('dd MMM HH:mm').format(DateTime.parse(flight['departureTime'])), style: const TextStyle(fontSize: 11, color: Colors.grey)),
                  ],
                ),
              )
            ],
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(flight['arrivalAirport'], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis),
                    Text(DateFormat('dd MMM HH:mm').format(DateTime.parse(flight['arrivalTime'])), style: const TextStyle(fontSize: 11, color: Colors.grey)),
                  ],
                ),
              )
            ],
